...
import com.oracle.oep.order.ebm.transform.ProcessCallDivertOrderEbmMapping;
...
public class ProcessCallDivertOrderFulfillmentProcessor {

	OepOrder oepOrderDO = null;
	ProducerTemplate template = null;
	
	public static final Logger log = LogManager.getLogger(ProcessCallDivertOrderFulfillmentProcessor.class); 	
	 ...
	 * 
	 */
	public void processCallDivertOrders(final OrderEntryRequest request,final Exchange exchange, final String oepOrderAction, final String orderPayload, final String oepOrderNumber, final String oepServiceType) throws Exception {
		...
		try{
			...
			String serviceNo = request.getProcessCallDivertManagment().getServiceNo();
			String channelName = request.getProcessCallDivertManagment().getChannelName();
			String userName = ProcessModifyOrderEbmHelper.getCsrId(request, oepOrderAction);

			//generate Order DataObject's		
			oepOrderDO = getOepOrderDO(request, oepOrderAction, oepOrderNumber, orderPayload, oepServiceType, channelName);
			
			ProcessCallDivertOrderEbmMapping callDivertOrderEbmMapping = new ProcessCallDivertOrderEbmMapping();
			CreateOrder salesOrderEbm = callDivertOrderEbmMapping.createOSMOrderEBM(request,oepOrderDO,  oepOrderAction, oepOrderNumber,accountId);						
			...
			CUSOPSEARCHOutputFlist searchResult = getAccountDataFromBRM(template, serviceNo, null, channelName, userName, oepOrderNumber, oepServiceType);			
	 ...
	 * 
	 */
	private OepOrder getOepOrderDO(final OrderEntryRequest request,final String oepOrderAction, final String oepOrderNumber, final String orderPayload, final String oepServiceType , final String channelName ){
