...
public class ProcessFcaOrderProcessor {

	public static final Logger log = LogManager.getLogger(ProcessFcaOrderProcessor.class);
	 ...
	 * 
	 */
	public void processFcaOrder(final FCA request, final Exchange exchange,
		...
		ProcessFcaAbmMapping fcaAbmMapping = new ProcessFcaAbmMapping();

		log.info("Calling abm mappping " + (String) template.requestBody("direct:convertxmlToString", request));
		
		abmRequest = fcaAbmMapping.createOrderEntryRequest(request, brmOrderAction, brmJmsTimeStamp);
